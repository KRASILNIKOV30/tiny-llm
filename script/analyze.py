...
def export_for_ai(summary_df):
    ...
    output_json = {
        ...
        "experiment_shorthands": {
            "Baseline": "Исходная модель без повреждений.",
            "L: x-y": "Layer Skipping: Полное отключение слоев с индексами от x до y-1. Вектор состояния проходит сквозь них без изменений.",
            "MLP: x-y": "MLP Severing: Отключение блоков Feed-Forward в слоях x-(y-1). Модель теряет доступ к 'базе знаний' на этих этапах.",
            "RoPE: x-y": "RoPE Mutilation: Отключение позиционного кодирования в слоях x-(y-1). Модель воспринимает токены как 'мешок слов'.",
            "H: L:T:I": "Head Zeroing: Отключение конкретной головы. L=слой, T=тип (q или kv), I=индекс. Помните: отключение одной KV-головы ослепляет сразу 7 Q-голов."
        },
        "metrics_guide": {
            "perplexity": "Перплексия на WikiText. Рост значения = деградация языковой модели.",
            "induction_score": "Способность к копированию паттернов. Значения ближе к 1.0 — идеальная работа индукционных голов.",
            "Acc %": "Точность выполнения задач (LAMA, MCQA, BLiMP, Passkey). 100% — идеальный результат."
        },
